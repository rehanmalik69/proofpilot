...
function WorkspaceSidebar({ caseId, status, inputId }: WorkspaceSidebarProps) {
  return (
    <>
      <EvidenceUploadPanel caseId={caseId} inputId={inputId} />
      ...
      <Card className="surface border-white/80 shadow-[0_22px_48px_rgba(15,23,42,0.05)]">
        ...
        <CardContent className="space-y-3.5 sm:space-y-4">
          <p className="text-[13px] leading-6 text-slate-600 sm:text-sm sm:leading-7">
            Analyze Evidence runs Groq when it is configured and available, then falls back
            automatically to ProofPilot&apos;s local rules-based analysis engine if needed. The
            result is saved to the case record and rendered below as ProofPilot&apos;s intelligence
            layer.
          </p>
    ...
    <section className="surface-strong overflow-hidden rounded-[1.75rem] border border-white/80 sm:rounded-[2rem]">
      <div className="space-y-5 p-5 sm:space-y-6 sm:p-8 lg:p-10">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <Badge>{disputeType}</Badge>
          <Badge variant={getCaseStatusVariant(status)}>{getCaseStatusLabel(status)}</Badge>
          <Badge variant="success">{analysisReady ? "Analysis ready" : "Awaiting analysis"}</Badge>
        </div>
...
function EmptyAnalysisState({ caseId }: EmptyAnalysisStateProps) {
  return (
    <Card className="surface border-white/80">
      ...
      <CardContent className="space-y-4 sm:space-y-5">
        <p className="text-sm leading-6 text-slate-600 sm:text-base sm:leading-7">
          ...
          chronology, extracted facts, scoring, proof gaps, and a polished draft you can refine,
          using Groq when available with a built-in local fallback path.
        </p>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[1.35rem] border border-slate-200/80 bg-slate-50/85 p-4 sm:rounded-[1.5rem] sm:p-5">
            ...
            <p className="mt-2.5 text-sm font-semibold text-slate-950">A clear dispute chronology</p>
          ...
          <div className="rounded-[1.35rem] border border-slate-200/80 bg-slate-50/85 p-4 sm:rounded-[1.5rem] sm:p-5">
            ...
            <p className="mt-2 text-[13px] leading-6 text-slate-600 sm:text-sm sm:leading-7">
              Merchant, amount, dates, case status, and evidence count rendered as a clean
              snapshot.
            </p>
          </div>
          <div className="rounded-[1.35rem] border border-slate-200/80 bg-slate-50/85 p-4 sm:rounded-[1.5rem] sm:p-5">
            ...
            <p className="mt-2 text-[13px] leading-6 text-slate-600 sm:text-sm sm:leading-7">
              Get a prioritized checklist of documents or screenshots that would strengthen the
              file.
            </p>
          </div>
          <div className="rounded-[1.35rem] border border-slate-200/80 bg-slate-50/85 p-4 sm:rounded-[1.5rem] sm:p-5">
    ...
    <div className="mx-auto flex w-full max-w-[90rem] flex-col gap-5 px-4 pb-10 pt-4 sm:gap-7 sm:px-6 sm:pb-16 sm:pt-8 lg:gap-8 lg:px-8 xl:px-10">
      ...
      <div className="space-y-5 sm:space-y-6 lg:hidden">
        <OverviewSection
          ...
          disputeType={caseItem.dispute_type}
          status={caseItem.status}
          merchantName={caseItem.merchant_name}
          transactionAmount={caseItem.transaction_amount}
          incidentDate={caseItem.incident_date}
          createdAt={caseItem.created_at}
          issueDescription={caseItem.issue_description}
        ...
        <div className="space-y-4 sm:space-y-5">
          <WorkspaceSidebar
            caseId={caseItem.id}
            status={caseItem.status}
            inputId="evidence-upload-mobile"
          />
        </div>
        ...
        {analysis ? (
          ...
          <EmptyAnalysisState caseId={caseItem.id} />
        )}
      </div>

      <div className="hidden lg:flex lg:items-start lg:gap-8">
        <div className="min-w-0 flex-1 space-y-6 xl:space-y-8">
          <OverviewSection
            title={caseItem.title}
            disputeType={caseItem.dispute_type}
            status={caseItem.status}
        ...
        <aside className="w-[22rem] flex-none xl:w-[24rem]">
          <div className="sticky top-8 space-y-5 xl:space-y-6">
            <WorkspaceSidebar
              caseId={caseItem.id}
              status={caseItem.status}
              inputId="evidence-upload-desktop"
            />
          </div>
        </aside>
      </div>
